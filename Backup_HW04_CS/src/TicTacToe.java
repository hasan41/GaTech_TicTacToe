...
public class TicTacToe {
  ...
  * @param p1 - Selects player1 as a string inside the getInput method passed in the location class
  * @param p2 - If the checkplayer is not an even integer then p2 
  * increment player every time after player 1 took its turn, so player 2 can make its move
  */
        ...
        while (board.getGameState() == GameState.ONGOING) {
            if (checkplayer % 2 == 0) {
                ...
                if (board.locatelocation(row, column)) {
                    board.find(location, 'X');
                    System.out.println(board);
                    checkplayer = checkplayer + 1; //increment player every time after player 1 took its turn.
                }
